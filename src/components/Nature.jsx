...
export default function Nature({ showNatureSounds, setShowNatureSounds, crossfaderValue  }) {
  ...
  const toggleNatureSound = async (soundType) => {
    ...
    try {
      if (!wasActive) {
        ...
        // Birds için 1.5x boost, diğerleri için normal volume
        const multiplier = soundType === 'birds' ? 1.5 : 1.0
        const crossfaderEffect = crossfaderValue / 100 // 🔹 Crossfader etkisi
  ...
  const handleVolumeChange = (soundType, volume) => {
    ...
    if (audio) {
      // Crossfader etkisi (nature için 0-100 arası değer)
      const crossfaderEffect = (100 - crossfaderValue) / 100 // 0 = hiç nature, 1 = tam nature
      
      // Birds için 1.5x boost, diğerleri için normal
      const multiplier = soundType === 'birds' ? 1.5 : 1.0
      
      // Final volume = kullanıcının slider değeri × crossfader etkisi × boost
